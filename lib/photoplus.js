...
define(['jquery', 'flight/lib/component'], function($, defineComponent) {
  return defineComponent(function() {
    ...
    this.setupGallery = function() {
      ...
      $gallery.width(this.total * this.imageWidth);
    ...
    this.markCurrentImage = function() {
      console.log("current", this.current());
      this.select('gallerySelector .current').each(function(a, b) {
        console.log(a, b);
        return this.removeClass('current');
      });
      console.log(this.select('gallerySelector').find("nth-child(" + (this.current()) + ")"));
      return this.select('gallerySelector').find("nth-child(" + (this.current()) + ")").addClass('current');
    };
    this.browse = function(direction, cb) {
      var options;
      console.log(this.select('gallerySelector').find("nth-child(" + (this.current()) + ")").attr('src'));
      if (!this.processing) {
        ...
        return this.select('gallerySelector').animate(options, 400, (function(_this) {
          return function() {
            _this.markCurrentImage();
            console.log(_this.select('gallerySelector').find("img:nth-child(" + (_this.current()) + ")").attr('src'));
