...
define(['jquery', 'flight/lib/component', 'image-helper'], function($, defineComponent, imageHelper) {
  var photoPlus;
  photoPlus = function() {
    this.defaultAttrs({
    ...
    this.populateGallery = function(media) {
      ...
      $(this.photos.slice(1, 5)).each((function(_this) {
        return function(index, photo) {
          ...
          html += "<img src='" + (imageHelper.url(_this.addSize(photo.path))) + "' ";
    ...
    this.getListingId = function() {
      return this.$node.closest('.spotlight, .result').data('listing_id');
    };
